    ...
    <div className={`flex items-center gap-1.5 md:gap-3 ${className}`}>
      {onExercisePause && (
        <button
          ...
          className="px-2 py-1.5 md:px-3 md:py-2 bg-orange-500/80 rounded-lg hover:bg-orange-600/80 transition text-white flex-shrink-0 flex items-center gap-1 md:gap-2"
        ...
        >
          {isExercisePaused ? (
            <>
              ...
              <span className="text-xs md:text-sm font-semibold hidden sm:inline">운동 재개</span>
            </>
          ) : (
            <>
              <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span className="text-xs md:text-sm font-semibold hidden sm:inline">운동일시정지</span>
            </>
          )}
        </button>
      )}
      <button
        onClick={handleMute}
        className="p-1.5 md:p-2 bg-black/70 rounded-lg hover:bg-black/80 transition text-white flex-shrink-0"
      ...
      <div className="flex items-center gap-1.5 md:gap-2 flex-1 min-w-0">
        <input
          ...
          className="flex-1 h-1.5 md:h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer slider"
        />
        <span className="text-white text-xs md:text-sm w-8 md:w-10 text-right flex-shrink-0">
